...
// Records the UTR as pending. UPI has no server-side confirmation callback in
// this flow, so credits are granted only after an admin verifies the bank
// statement and approves the payment.
  ...
  try {
    const payment = await db.transaction(async (tx) => {
      const [inserted] = await tx
        ...
        .values({ userId, credits, amountInr: credits, utr, status: "pending", updatedAt: new Date() })
        .returning();
      if (!inserted) throw new Error("PAYMENT_INSERT_FAILED");
      return { payment: inserted };
    });

    logger.info({ paymentId: payment.payment.id, userId, credits }, "UPI payment submitted for admin verification");
    ...
    res.json({ ok: true, paymentId: payment.payment.id, status: "pending", credits });
  } catch (err) {
