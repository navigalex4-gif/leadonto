...
Deepgram realtime accepts browser MediaRecorder WebM only when the live request declares the WebM container; it can still close after useful interim text without sending `speech_final`. Treat `error`/`closed` as a handoff to final blob STT, not as a terminal UI state. The relay should promote its latest useful interim to a final transcript on provider error/close so mobile disconnects cannot drop an otherwise valid answer.
