...
## Approach: CSS mouth-band overlay on the real photo
- While `isSpeaking && imageSrc`, a `PhotoMouth` overlay mounts: a copy of the SAME photo, masked to a TIGHT band over the mouth/chin with a feathered CSS gradient mask, applying only a tiny audio-driven scale within that band.
...
- **Keep the entire portrait stationary — no idle breathing animation and no `translateY` on the photo copy.** A large `scaleY` reads as "the face is being stretched/pulled", so use only a tiny scale inside the tight mouth mask. Real phoneme sync on a static photo isn't achievable here (see rejected approaches), so subtlety — not bigger motion — is the only lever.
...
- **How to apply:** if the effect ever looks like the body is "breathing", remove vertical translation/idle animation first and raise the mask's lower edge; do not move the whole photo.
