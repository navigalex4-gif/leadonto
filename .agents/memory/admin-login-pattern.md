...
description: /auth/admin-login uses a default admin username and a securely configured password or SHA-256 hash.
---
...
2. Server reads `ADMIN_USERNAME` (default: "admin") and either `ADMIN_PASSWORD_HASH` or `ADMIN_PASSWORD` from env
3. Computes `SHA-256(password)` and compares hex strings; the hash secret takes precedence
4. On match: seeds `admin@edubharat.in` user in DB if not exists, sets session

**Env vars needed:**
- `ADMIN_USERNAME` = "admin" (optional)
- `ADMIN_PASSWORD_HASH` = SHA-256 hex of the password (preferred secret)
- `ADMIN_PASSWORD` = password (setup convenience fallback when no hash is configured)
